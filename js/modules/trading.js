/**
 * trading.js
 * Handles opening, closing positions, P&L calculations.
 * Versione stabile senza modifica/chiusura parziale via prompt.
 */
import { simState, getCurrentAssetConfig, getCurrentTimeframeSeconds } from '../state.js';
import { CONFIG } from '../config.js';
import * as Utils from './utils.js';
import * as UIModule from './ui.js'; // Importa UIModule
 ...
 */
export async function openPosition(/** @type {'BUY'|'SELL'} */ type) { // Rimuovi parametri non usati
    ...
    // Ottieni input dal PANNELLO LATERALE
    const inputs = UIModule.getSidePanelRiskInputs(); // Usa la funzione corretta
    ...
    if (isNaN(inputs.size) || inputs.size <= 0) {
    ...
    try {
        if (inputs.method === 'atr') { // Calcolo ATR
            ...
            if(inputs.slValue < CONFIG.MIN_ATR_SL_MULTIPLE) { UIModule.showFeedback(`Mult. SL ATR < min.`, "error"); return false; }
            if(inputs.tpValue < CONFIG.MIN_ATR_TP_MULTIPLE) { UIModule.showFeedback(`Mult. TP ATR < min.`, "error"); return false; }
...
/** Modifica SL/TP - Funzione MANTENUTA ma NON chiamata dall'UI attuale */
export async function modifyPosition(positionId, newSlPrice, newTpPrice) {
    // Questa logica rimane valida se si reintroduce un pulsante/modo per chiamarla
    ...
    // ... (resto della logica di validazione e aggiornamento come prima) ...
     ...
     console.warn("Modify position called, but no UI trigger currently exists in this stable version.");
}

/** Chiude una posizione (totale o parziale - logica parziale semplificata). */
export async function closePosition(positionId, reason = 'manual', sizeToCloseUnits = null) { // sizeToCloseUnits ora è null per default (chiusura totale)
    const posIndex = simState.openPositions.findIndex(p => p.id === positionId);
    ...
    // Determina size da chiudere: default a totale
    const actualSizeToClose = (sizeToCloseUnits === null || isNaN(sizeToCloseUnits) || sizeToCloseUnits >= pos.size)
                              ? pos.size : Math.max(0, sizeToCloseUnits);

    if (actualSizeToClose <= 0) return UIModule.showFeedback(`Size chiusura N/V Pos ${pos.id}.`, "warn");

    const isPartialClose = actualSizeToClose < pos.size; // Sarà false se sizeToCloseUnits è null
    ...
    // --- Log Storico ---
    // Passa size chiusa e flag parziale
    ...
    let feedbackMsg;
    if (isPartialClose) { // Logica chiusura parziale MANTENUTA ma non attivata dall'UI attuale
        ...
        feedbackMsg = `Chiusura Parziale Pos ${pos.id} (...) P&L: ${Utils.formatCurrency(pnl)}. Rim.: ${Utils.formatVolume(remainingLots, pos.asset)}`;
         // Ridisegna le linee per aggiornare il P&L nel titolo (la size è cambiata implicitamente per il prossimo tick)
         ChartModule.drawPositionLines(simState.openPositions[posIndex]);
...
export function calculateLivePnl(pos, currentBidPrice) { /* ... come prima ... */ }
/** Checks if SL or TP was hit within a bar. */
export function checkSLTP(pos, barHigh, barLow) { /* ... come prima ... */ }
