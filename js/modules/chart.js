        ...
        simState.charts.main = LightweightCharts.createChart(ui.chartContainer, \{\
            ...
            rightPriceScale: \{ // Configure main price scale\
                ...currentChartThemeOptions.rightPriceScale, // Inherit theme border etc.\
                 ...
                 // precision: assetConf.pricePrecision, // Let formatter handle precision\
                 borderVisible: true,\
            \},\
            localization: \{\
...
\
        ...
        simState.series.atr = simState.charts.main.addLineSeries(\{\
            ...
            priceFormat: \{ // How to format the ATR value display on the scale/tooltip\
                ...
                precision: assetConf.pricePrecision > 2 ? 2 : 4,\
            \},\
...
export function removeAllPositionLines() \{\
     ...
     \});\
     // simState.tradeLines should be empty now\
\}\
\
/**\
...
\
    ...
    while (simState.equityHistory.length >= CONFIG.EQUITY_HISTORY_MAX_POINTS) \{\
        simState.equityHistory.shift(); // Remove the oldest point\
    \}\
...
\
     ...
     \}\
      ...
      Object.values(simState.openPositions).forEach(position => \{\
...
\
     // Reset Equity Chart Data to initial state\
      if (simState.charts.equity && simState.series.equityCurve) \{\
          try \{\
              const initialEquity = [\{ time: Math.floor(Date.now() / 1000), value: simState.capital \}];\
              simState.equityHistory = [...initialEquity]; // Reset history array\
              simState.series.equityCurve.setData(initialEquity); // Set chart data\
 ...
 function themeVar(varName) \{\
    ...
    // Provide sensible fallbacks based on common theme structure\
     switch (varName) \{\
        ...
        case 'text-primary': return simState.selectedTheme === 'light' ? '#131722' : '#d1d4dc';\
...
\
          // Check if chart instances and containers exist and have dimensions\
