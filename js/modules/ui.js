...
const elementsToCache = [ // ASSICURATI CHE QUESTA LISTA SIA PRESENTE E CORRETTA
...
export function initialize() {
    ...
    let allFound = true;
    // USA LA VARIABILE elementsToCache DEFINITA SOPRA
...
async function handleExecuteOrder() { /* ... come prima (chiama TradingModule.openPosition) ... */
    try { const Trading=await import('./trading.js'); await Trading.openPosition(simState.orderModalType); closeOrderModal(); }
...
export function updateEstimatedRisk(riskAmount, riskPercent, isModal = false) {
    const displayElement = isModal ? ui.modalEstimatedRiskDisplay : null; // Solo Modale ora
...
// Rinomitata e Esportata perché chiamata dall'esterno (handleModalRiskMethodChange)
