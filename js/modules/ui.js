...
export function initialize() {
    ...
    if (ui.atrVisibleToggle) {
        ui.atrVisibleToggle.checked = simState.isAtrVisible;
    }
        ...
        if (!isNaN(positionId) && !button.disabled) {
            ...
            try {
                ...
                await TradingModule.closePosition(positionId, 'manual');
                 ...
                 showFeedback("Errore chiusura posizione.", "error");
            }
        }
    }
}
...
/**
 * Applies theme class to body and updates toggle button icon.
 * @param {'dark'|'light'} theme
 */
function applyTheme(theme) {
     document.body.className = `theme-${theme}`;
     if (ui.themeToggleBtn) ui.themeToggleBtn.textContent = theme === 'dark' ? '☀️' : '🌙';
     simState.selectedTheme = theme;
}
...
/**
 * Handles ATR Visibility checkbox change.
 */
async function handleAtrVisibilityChange(event) {
    const isVisible = event.target.checked;
    simState.isAtrVisible = isVisible;
    saveSettings(); // Save the preference
    try {
        const ChartModule = await import('./chart.js');
        ChartModule.setAtrVisibility(isVisible); // Update chart
...
function saveSettings() {
     Utils.saveToLocalStorage(CONFIG.LOCALSTORAGE_SETTINGS_KEY, {
         ...
         isAtrVisible: simState.isAtrVisible // Save ATR visibility
     });
}

// --- UI Update Functions (Existing code from previous answer) ---
...
export function updateStatsBar() { /* ... come prima ... */
    ...
    const atrDisplayValue = !isNaN(simState.currentATR) ? (simState.currentATR * assetConf.atrDisplayMultiplier).toFixed(assetConf.pricePrecision > 2 ? 1 : 2) : '--';
...
export function updateEstimatedRisk(riskAmount, riskPercent) { /* ... come prima ... */
...
export function updateRiskInputVisibility(selectedMethod) { /* ... come prima ... */
...
export function setControlsEnabled(enabled) { /* ... come prima ... */
    ui.assetSelect?.toggleAttribute('disabled', !enabled); ui.timeframeSelect?.toggleAttribute('disabled', !enabled); ui.sizeInput?.toggleAttribute('disabled', !enabled); ui.slPipsInput?.toggleAttribute('disabled', !enabled); ui.tpPipsInput?.toggleAttribute('disabled', !enabled); ui.slAtrMultiInput?.toggleAttribute('disabled', !enabled); ui.tpAtrMultiInput?.toggleAttribute('disabled', !enabled); ui.btnBuy?.toggleAttribute('disabled', !enabled); ui.btnSell?.toggleAttribute('disabled', !enabled); document.querySelectorAll('input[name="riskMethod"]').forEach(radio => radio.disabled = !enabled); ui.clearHistoryBtn?.toggleAttribute('disabled', !enabled); ui.atrVisibleToggle?.toggleAttribute('disabled', !enabled); // Disable ATR toggle too
    const closeButtons = ui.openPositionsTableBody?.querySelectorAll('.close-pos-btn'); closeButtons?.forEach(btn => btn.disabled = !simState.isRunning);
}
export function getCurrentRiskInputs() { /* ... come prima ... */
    const method = simState.selectedRiskMethod; let slValue = NaN, tpValue = NaN; try { if (method === 'atr') { slValue = parseFloat(ui.slAtrMultiInput.value); tpValue = parseFloat(ui.tpAtrMultiInput.value); } else { slValue = parseFloat(ui.slPipsInput.value); tpValue = parseFloat(ui.tpPipsInput.value); } } catch (e) {} let size = NaN; try { size = parseFloat(ui.sizeInput.value); } catch (e) {} return { method, size, slValue, tpValue };
}